...
namespace FxSsh.Services
{
    public class ConnectionService : SshService, IDynamicInvoker
    {
        ...
        private void HandleMessage(ChannelOpenMessage message)
        {
            switch (message.ChannelType)
            {
                ...
                case "forwarded-tcpip":
                    ...
                    break;
                default:
                    _session.SendMessageAsync(new ChannelOpenFailureMessage
                    {
            ...
            switch (message.RequestType)
            {
                ...
                case "simple@putty.projects.tartarus.org":
                    ...
                    if (message.WantReply)
                    {
                        ...
                        _session.SendMessageAsync(new ChannelSuccessMessage { RecipientChannel = c.ClientChannelId }).Wait();
                    }
                    break;
                case "winadj@putty.projects.tartarus.org":
                    //https://tartarus.org/~simon/putty-snapshots/htmldoc/AppendixF.html
                    var channel = FindChannelByServerId<SessionChannel>(message.RecipientChannel);
                    _session.SendMessageAsync(new ChannelFailureMessage { RecipientChannel = channel.ClientChannelId }).Wait();
                    break;
                ...
                default:
                    if (message.WantReply)
                        _session.SendMessageAsync(new ChannelFailureMessage
                        {
        ...
        private void HandleMessage(EnvMessage message)
        {
            ...
            if (message.WantReply)
                _session.SendMessageAsync(new ChannelSuccessMessage { RecipientChannel = channel.ClientChannelId }).Wait();
        }
        ...
        private SessionChannel HandleChannelOpenMessage(ChannelOpenMessage message)
        {
            ...
            _session.SendMessageAsync(msg).Wait();
            return channel;
        }
        ...
        internal async Task HandleMessageCoreAsync(ConnectionServiceMessage message)
        {
